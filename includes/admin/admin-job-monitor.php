...
class ContaiAdminJobMonitor {
	...
	private function renderAutoRefreshScript(): void {
		wp_enqueue_script(
			...
			plugin_dir_url( __DIR__ ) . 'admin/assets/js/tai-job-monitor-refresh.js',
			array(),
			filemtime( plugin_dir_path( __DIR__ ) . 'admin/assets/js/tai-job-monitor-refresh.js' ),
			true
		);
	}
...
function contai_render_job_monitor_page() {
	...
	$monitor = new ContaiAdminJobMonitor();
	$monitor->render();
}
